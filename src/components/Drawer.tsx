                    ...
                    <div className="px-2.5 py-2">
                      ...
                      <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                        {a.geotagged && a.lat != null && a.lng != null && (
                          <a href={mapsLink(a.lat, a.lng)} target="_blank" rel="noreferrer" className={`inline-flex items-center gap-1 rounded-sm px-1.5 py-0.5 font-mono text-[9px] font-bold uppercase tracking-wider transition ${a.geoSource === 'device' ? 'bg-cyanx-500/12 text-cyanx-400 hover:bg-cyanx-500/25' : 'bg-tealx-500/12 text-tealx-400 hover:bg-tealx-500/25'}`} title={`Geotagged at ${fmtCoord(a.lat, a.lng)} · via ${a.geoSource === 'device' ? 'device location' : 'photo EXIF'}`}>
                            <I n="pin" className="h-2.5 w-2.5" sw={2.4} /> {fmtCoord(a.lat, a.lng)}{a.geoSource === 'device' ? ' · device' : ''}
                          </a>
                        )}
                        {!(a.geotagged && a.lat != null && a.lng != null) && (
  ...
  return (
    <div className="mt-3">
      ...
      <button onClick={() => fileRef.current?.click()} disabled={busy}
        ...
        <I n="cam" className="h-4 w-4" /> {busy ? 'Reading photo geotags…' : 'Add JPG / PDF evidence (geotagged photos are mapped)'}
      </button>
