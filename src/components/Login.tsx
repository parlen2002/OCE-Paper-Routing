...
export function Login() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    window.setTimeout(() => {
      const ok = login(u, p);
      if (!ok) {
        setErr('Access denied — check the username and password against the authorized list.');
        setShakeKey((k) => k + 1);
        setBusy(false);
      } else {
    ...
    <div className="flex min-h-screen bg-ink-950">
      ...
      <div className="relative hidden w-[54%] flex-col justify-between overflow-hidden border-r border-ink-700/60 bg-ink-900 p-10 lg:flex">
        ...
        <div className="relative max-w-xl">
          ...
          <h1 className="font-display text-[64px] font-bold uppercase leading-[0.95] tracking-wide text-mist-50 xl:text-[76px]">
            Every paper,
            <br />
          ...
          <p className="mt-5 max-w-md text-[14px] leading-relaxed text-mist-300">
            Work orders, permits, memos and complaints move across nine divisions under two
            supervising clusters. Each hand-off is stamped into the chain of custody — so a paper
            never disappears between floors.
          </p>
        ...
        <div className="relative flex items-center gap-6 font-mono text-[10.5px] uppercase tracking-[0.2em] text-mist-500">
          ...
          <span><b className="text-mist-200">02</b> supervising clusters</span>
          <span className="h-3 w-px bg-ink-600" />
      ...
      <div className="bg-blueprint flex flex-1 items-center justify-center p-6">
        <div className="w-full max-w-md">
          <div className="mb-6 flex items-center gap-3 lg:hidden">
            ...
            <div>
              <p className="font-display text-base font-bold uppercase tracking-wider text-mist-50">City Engineering Office</p>
              <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-mist-400">Puerto Princesa City</p>
            </div>
          </div>

          <div key={shakeKey} className={`${err ? 'anim-shake' : 'anim-fade-up'}`}>
            <div className="rounded-xl border border-ink-600 bg-ink-900/90 p-7 shadow-[0_30px_80px_-20px_rgba(0,0,0,0.75)]">
              ...
              <h2 className="mt-1.5 font-display text-[30px] font-bold uppercase leading-none tracking-wide text-mist-50">
                Sign in to the floor
              </h2>
              <p className="mt-2 text-[12.5px] leading-relaxed text-mist-400">
                Supervisors see the whole office. Division accounts see their own queue and the
                trail of everything that passed through it.
              </p>
