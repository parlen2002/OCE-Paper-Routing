            ...
            <div className={`rounded-lg border bg-ink-950/40 p-2.5 ${err(recipients.length === 0) ? 'border-redx-500/70' : 'border-ink-600'}`}>
              ...
              <div className="mb-2.5 flex flex-wrap gap-1.5">
                {DESKS.map((d) => {
                  ...
                  return (
                    <button type="button" key={d.id} onClick={() => toggle(d.id)} title={`${d.name} — ${d.head}`}
                      className={`inline-flex items-center gap-1.5 rounded-md border px-2 py-1.5 font-mono text-[10px] font-bold uppercase tracking-wider transition active:scale-[0.97] ${on ? 'border-amberx-500/70 bg-amberx-500/15 text-amberx-400' : 'border-ink-600 bg-ink-850 text-mist-500 hover:border-amberx-500/40 hover:text-mist-200'}`}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {[...DIVISIONS, ...CROSS_UNITS].map((d) => {
                  const on = recipients.includes(d.id);
                  return (
                    <button type="button" key={d.id} onClick={() => toggle(d.id)} title={d.name}
                      className={`inline-flex items-center gap-1.5 rounded-md border px-2 py-1.5 font-mono text-[10px] font-bold uppercase tracking-wider transition active:scale-[0.97] ${on ? 'border-cyanx-500/70 bg-cyanx-500/12 text-cyanx-400' : 'border-ink-600 bg-ink-850 text-mist-500 hover:border-cyanx-500/40 hover:text-mist-200'}`}>
                      {on && <I n="check" className="h-3 w-3" sw={2.6} />} {d.code}
                    </button>
                  );
                })}
