...
function ProfileModal() {
  const { user, db, setProfileOpen, changePassword, requestPasswordReset } = useStore();
        ...
        <div className="border-t border-ink-700/70 p-3">
          <div className="flex items-center gap-2.5 rounded-md bg-ink-850 px-2.5 py-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate font-mono text-[9.5px] uppercase tracking-wider text-mist-500">{user.shortTitle ?? (user.role === 'admin' ? 'Administrator' : user.role === 'supervisor' ? 'Dept. Head' : user.role === 'moderator' ? 'Moderator' : user.role === 'division' ? 'Division Head' : user.role === 'joborder' ? 'Job-Order' : 'Employee')}</p>
            </div>
      ...
      <div className="pl-[228px]">
        <header className="sticky top-0 z-30 flex h-[58px] items-center gap-4 border-b border-ink-700/70 bg-ink-950/85 px-5 backdrop-blur">
          ...
          <div className="ml-auto flex items-center gap-2.5">
            <button onClick={() => setReportOpen(true)} className="rounded-md border border-ink-600 bg-ink-850 p-2.5 text-mist-300 transition hover:border-flare-500/70 hover:text-flare-400" title="Print routing report — daily / weekly / monthly">
