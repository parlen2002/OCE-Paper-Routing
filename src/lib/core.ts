...
export interface Paper {
  ...
  /** Completion rate 0–100 — adjustable per paper; printed on reports. */
  progress?: number;
}
...
export const cityEngineerName = (users: User[]): string =>
  users.find((u) => u.id === 'u-sup1')?.name ?? 'Engr. Aries S. Grande';
...
export async function buildAttachments(
  files: FileList | File[],
  by: string
): Promise<{ atts: Attachment[]; skipped: string[] }> {
...
export function freshSeed(): DB {
  ...
  const pdfBudget = makeStubPdf('Supplemental Budget Request — H2 2026', [
    ...
    'Status: Under final review at the City Engineer desk',
    '',
  ...
  const pdfSurvey = makeStubPdf('Survey Request — Lot Verification, Brgy. San Pedro', [
    ...
    'Purpose: clearance for proposed drainage easement',
    'Status: Field verification complete — submitted to division head',
    '',
    'Prepared by: Survey and Mapping Division',
  ]);
  ...
  const papers: Paper[] = [
    {
      ...
      attachments: [
        ...
        att('work-order-0141.pdf', 'pdf', pdfBudget, now - 2.2 * D, 'Ms. Carol Estrella', undefined, '96 KB'),
      ],
      custody: [
        c(now - 3 * D, CAROL, 'created', 'Logged into the system and transmitted to Construction Division', { toDivisionId: 'const' }),
    ...
    },
    {
      ...
      custody: [
        c(now - 1.1 * D, CAROL, 'created', 'Logged into the system and transmitted to Maintenance Division', { toDivisionId: 'maint' }),
    ...
    },
    {
      ...
      custody: [
        c(now - 2.6 * D, CAROL, 'created', 'Logged into the system and transmitted to Survey and Mapping Division', { toDivisionId: 'survey' }),
    ...
    },
    {
      ...
      custody: [
        c(now - 2.2 * D, CAROL, 'created', 'Logged into the system and transmitted to Planning Design and Programming Division', { toDivisionId: 'plan' }),
    ...
    },
    {
      ...
      attachments: [],
      custody: [
        c(now - 0.3 * D, CAROL, 'created', 'Logged into the system and transmitted to Administrative Division', { toDivisionId: 'admin' }),
      ...
      createdAt: now - 0.3 * D, updatedAt: now - 0.15 * D, byId: 'u-admindiv', byName: CAROL,
    ...
    },
    {
      ...
      custody: [
        c(now - 5 * D, CAROL, 'created', 'Logged into the system and transmitted to Inspectorate Team', { toDivisionId: 'insp-team' }),
    ...
    },
    {
      ...
      attachments: [],
      custody: [
        c(now - 6 * D, CAROL, 'created', 'Logged into the system and transmitted to Public Services Division', { toDivisionId: 'psd' }),
      ...
      createdAt: now - 6 * D, updatedAt: now - 1.5 * D, byId: 'u-admindiv', byName: CAROL,
    ...
    },
    {
      ...
      custody: [
        c(now - 1.4 * D, CAROL, 'created', 'Logged into the system and transmitted to the City Engineer desk', { toDivisionId: 'desk-ce' }),
        c(now - 0.9 * D, CE, 'stage', 'Line items under final review', { stage: 'verification' }),
      ],
      createdAt: now - 1.4 * D, updatedAt: now - 0.9 * D, byId: 'u-admindiv', byName: CAROL,
    ...
    },
    {
      ...
      attachments: [],
      custody: [
        c(now - 2.4 * D, CAROL, 'created', 'Logged into the system and transmitted to Documentation and Monitoring Team', { toDivisionId: 'docmon' }),
    ...
    },
    {
      ...
      attachments: [],
      custody: [
        c(now - 0.5 * D, CAROL, 'created', 'Citizen report logged into the system and transmitted to Subaybayan Team', { toDivisionId: 'subay' }),
    ...
    },
    {
      ...
      attachments: [],
      custody: [
        c(now - 1.1 * D, CAROL, 'created', 'Circular logged and addressed to all divisions and offices — acknowledgement required', { toDivisionId: 'desk-ce' }),
    ...
    },
    {
      ...
      attachments: [],
      custody: [
        c(now - 8 * D, CAROL, 'created', 'Logged into the system and transmitted to Materials Testing and Quality Control Division', { toDivisionId: 'mtqc' }),
...
/** Rebuilds the system log (per-user history) from custody trails, plus sign-in events. */
